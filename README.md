...
- 独立播放窗口；单次播放队列最多 300 条，由主进程去重、按库中当前记录解析并过滤缺失文件，固定播放器 URL 不携带业务 ID 或完整队列。窗口通过带序列号的类型化 IPC 快照与领域事件同步；`auto` 根据已缓存容器和 codec 保守选择 native/mpv，历史视频只在首次播放时懒补全 codec；Chromium 原生 → mpv → 系统默认播放器三级 fallback；播放控制、快捷键、上下部。
