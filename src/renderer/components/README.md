...
- `DuplicateGroupsPage.tsx`：分别展示同大小候选统计与“精确大小＋缓存时长”匹配组，支持分页、全局大小排序、当前页保留选择，以及从任一候选文件一键把其所在目录树设为优先保留范围。优先目录始终递归包含所有子目录；筛选会返回命中目录树的候选组及组内跨目录的全部文件，并优先推荐保留目录树中的一个文件。显式单组选择高于目录推荐。重复页不得为了判断而主动读取视频或计算指纹；UI 只能提交独立完整验证意图，永久删除仍由主进程的 SHA-256 授权与二次确认门禁控制。
