...
export function DuplicateDirectoryRanking({ options, onSelect }: {
  ...
  return <section className="duplicate-directory-ranking" aria-label="目录清理排行">
    <header><div><h3>目录清理排行</h3><p>选择要保留的目录，查看涉及的全部候选。目录包含子目录，各行数量不能相加。</p></div>
      <button className="duplicate-directory-ranking-toggle" type="button" aria-expanded={expanded} aria-controls="duplicate-directory-ranking-content" onClick={() => setExpanded((current) => !current)}>
        {expanded ? "收起" : "展开"}（{options.length.toLocaleString()} 个目录）
      </button></header>
    {expanded && <div id="duplicate-directory-ranking-content" className="duplicate-directory-ranking-content">
      <input aria-label="搜索排行目录" placeholder="搜索目录名称或路径" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} />
