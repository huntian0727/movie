...
const mainApi: VideoManagerApi = {
  ...
  exportVideoData: (query, selection) => ipcRenderer.invoke(channels.videoDataExport, query, selection),
  deleteVideoData: (query, selection) => ipcRenderer.invoke(channels.videoDataDelete, query, selection),
  listAssetCenterSources: (query: AssetCenterSourceQuery) => ipcRenderer.invoke(channels.assetCenterSources, query),
  searchPlaybackDiagnosticVideos: (query: PlaybackDiagnosticSearchQuery) => ipcRenderer.invoke(channels.playbackDiagnosticSearch, query),
  ...
  refreshMetadataFileSizes: (videoIds: string[]) => ipcRenderer.invoke(channels.libraryMetadataRefreshSizes, videoIds),
  ...
  scanDirectory: (request) => ipcRenderer.invoke(channels.folderScanDirectory, request),
