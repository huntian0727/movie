...
- `playbackMetadataEnricher.ts`：只为历史 ready 且 codec 为空、并且真正准备播放的视频懒补全编码信息；同视频并发合并，失败记录脱敏日志并保守走 mpv。禁止把它改成启动时或迁移时的全库回填。
...
- `mpvController.ts`/`playerRouting.ts`：外部播放与路由。`auto` 按容器+codec 保守选择；`native-first` 按受支持容器优先 native；`mpv-first` 始终 mpv。Renderer 的 native 失败回退与主进程 mpv→系统默认播放器回退必须保留。
